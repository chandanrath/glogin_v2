...
include_once '../includes/functions.php';
	...
	if($_POST['type']=='material')
	{
		...
		else if($_POST['type']=='subproduct')
		{
			...
			$productId = explode(",",$_POST['pid']);
			
			foreach($productId as $prodVal)
			{
				$SQL="SELECT id,product_id,opname,opcode FROM ri_subproduct WHERE status=1 and  product_id='".$prodVal."' ORDER BY opname ASC";	
				$result= mysqli_query($conn,$SQL);
				$count = mysqli_num_rows($result);
				
				if($count > 0)
				{
					$html .='<div class="custom-subprod'.$prodVal.'" style="background:#a8e3ef7a;font-weight:bold;">'.ProductValue($conn,$prodVal).'</div>';
					
					while($fetch_data=mysqli_fetch_array($result))
					{						
						$json[$prodVal][] = $fetch_data['opname'];
						
			$html .='<div class="custom-select-option-subprod"><input onChange="toggleFillColorSubProd(this);" class="custom-select-option-checkbox-subprod" id="custom-select-option-checkbox-subprod" type="checkbox" name="subproduct['.$prodVal.'][]" value="'.$fetch_data['id'].'" title="'.$fetch_data['opname'].'">'.$fetch_data['opname'].'</div>';
			...
			foreach($subproduct as $suvVal)
			{
				...
				if($count > 0)
				{
					...
					{						
						...
						$html .='<div class="custom-select-option-subprodtype" ><input onChange="toggleFillColorSubProdType(this);" class="custom-select-option-checkbox-subprodtype" id="custom-select-option-checkbox-subprodtype" type="checkbox"  name="subproducttype['.$fetch_data['subid'].'][]" value="'.$fetch_data['id'].'" title="'.utf8_encode($fetch_data['prodtype']).'">'.utf8_encode($fetch_data['prodtype']).'</div>';
		...
		else if($_POST['type']=='grades')
		{
			...
			//and  product_id IN(".$product.")
			 ...
			 $SQL="SELECT id,gradesname,gradescode FROM ri_grades WHERE status=1 and product_id IN(".$product.") and material_id ='".$_POST['mid']."' and subproduct_id=0 and equivalent_id=0 GROUP BY gradesname ORDER BY gradesname ASC";	
...
function ProductValue($conn,$pid)
{
	$SQL="SELECT prodname FROM ri_product WHERE status=1 and id ='".$pid."' ";	
	$result= mysqli_query($conn,$SQL);
	$count = mysqli_num_rows($result);
	
	$spdata=mysqli_fetch_array($result);
	
	return $spdata['prodname'];
